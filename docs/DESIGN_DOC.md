...
| FR-3  | **Search** — search shows by name via `GET /search/shows?q=`              | Assignment | Desktop search expands from the header; mobile uses `/search`. Typing queries TVMaze after debounce (see §5.2). Clearing the query stays on `/search` with the bar open; X or Escape returns to `/`. Mobile clear keeps the dedicated search prompt on `/search`.            |
...
| NFR-6  | **Responsive layout**                            | Usable from ~375px to desktop. Browse uses a compact cinematic featured card, one Your Next Watch rail, and a growing portrait-cover grid; mobile search has a dedicated page while desktop search expands from the header. Detail is a readable single column. Plan web + mobile UX first (`.cursor/rules/design-ux.mdc`). |
...
- `q === ''` on `/search` → dedicated search prompt (search query disabled).
...
- **URL sync:** search and status filter are mirrored in the query string on `/search` so views are shareable and survive reload/back/forward:
  - `q` — search text (omitted when empty)
  - `status` — `Running`, `Ended`, or `To Be Determined` (omitted when `all`)
  - Example: `/search?q=mock&status=Ended`
...
| Search then clear                       | Search hits mix into paginated browse rows                                   | `/search` and `/` are different routes. Clearing the query stays on `/search`; X/Escape returns to browse on `/`.                                      |
| Infinite scroll + search                | Sentinel never loads more than the single `/search/shows` payload            | Search keeps ranked API hits, then appends unique catalog matches from `GET /shows?page=` as the sentinel fires.                                       |
...
compact card-framed spotlight for the featured show, a portrait-cover Your Next Watch rail, and an infinite portrait-cover All Shows grid. Tokens
